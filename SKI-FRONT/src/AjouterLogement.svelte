  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        nom = ""; ville = ""; prix = 0; imageUrl = ""; capacite = 1; description = "";
      } else {
  ...
  <form on:submit|preventDefault={handleSubmit} class="flex flex-col gap-4">
    ...
    <div class="flex flex-col md:flex-row gap-4">
      <div class="flex flex-col gap-1 w-full">
        ...
        <input type="text" id="ville" bind:value={ville} required class="border p-2 rounded-lg" placeholder="Ex: Chamonix" />
      </div>

      <div class="flex flex-col gap-1 w-full">
        <label for="prix" class="font-semibold text-gray-700">Prix / nuit (€)</label>
        <input type="number" id="prix" bind:value={prix} required min="1" class="border p-2 rounded-lg" />
      </div>

      <div class="flex flex-col gap-1 w-full">
        <label for="capacite" class="font-semibold text-gray-700">Capacité</label>
        <input type="number" id="capacite" bind:value={capacite} required min="1" class="border p-2 rounded-lg" placeholder="Nb pers." />
      </div>
    </div>
